...
(function(xpl,undefined){
	...
	Vector.prototype.setFromEuler = function ( x, y, z, order ) {
    order = order || "XYZ";
	...
	Vector.prototype.toEuler = function(target,order){
    order = order || "YZX";
	...
	*/
	...
	//Calculate exact obliqueness of Earth
	//http://adsabs.harvard.edu/abs/1986A%26A...157...59L
...
function ecf_to_heliocentric(pos_ecf, jday){
	var earthPosition = xpl.SolarSystem(xpl.planets[2],jday)
	var scaled_pos_ecf= [pos_ecf.x,pos_ecf.z,-pos_ecf.y]
	
	var pointA = new Vector({x:scaled_pos_ecf[0],y:scaled_pos_ecf[1],z:scaled_pos_ecf[2],w:0})
	//pointA.setFromEuler(scaled_pos_ecf[0],scaled_pos_ecf[1],scaled_pos_ecf[2])
	var axisX = new Vector({  x: 1,  y: 0,  z: 0,  w: 0 })
	...
	//pointA.z*=-1
	pointA.rotate(axisX, angleC);
	//pointA.rotate(axisY, angleD);
	//pointA.y*=-1

	pointA.x+=earthPosition[0]
