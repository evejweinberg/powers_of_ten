...
loader.load("assets/milkywaypan_brunier.jpg",function (image){
...
var sunTexture = new THREE.Texture();
	loader.load( 'assets/Sun.jpg', function ( image ) {
...
var moonLabel
loader.load( "assets/Moon.jpg", function (image) {
				ptexture = new THREE.Texture();
				ptexture.image = image;
				ptexture.needsUpdate = true;
...
xpl.probePositions('voyager1',voyager1Positions,function(){
	...
	var modelPath = 'assets/3D/Voyager'
	...
	//mtlLoader.load( modelPath+'.mtl', function( materials ) {
		...
		loader.load( modelPath+'.obj', function ( object ) {
			...
			object.traverse( function ( child ) {
			if ( child instanceof THREE.Mesh ) {
				child.materials = new THREE.MeshLambertMaterial( {/*map: texture*/color:0xffffff, needsUpdate: true, side:THREE.DoubleSide} );
				child.geometry.computeVertexNormals();
				child.castShadow = true
				child.receiveShadow = true
			}
